...
# Mapping of NVSHMEM device-side API names to rocSHMEM equivalents.
#
# These are the *device* function symbols embedded in Triton extern wrappers
# (see torch/distributed/_symmetric_memory/_nvshmem_triton.py).  The _hip_device_api()
# helper in that module consults this dict at import time so that a single set of
# extern wrapper definitions works on both CUDA (NVSHMEM) and ROCm (rocSHMEM).
#
# Key differences between the two libraries:
#   - NVSHMEM uses _block suffix for CTA-scoped ops; rocSHMEM uses _wg (work-group).
#   - NVSHMEM nvshmemx_* prefix vs rocSHMEM rocshmem_* prefix.
#   - nvshmem_signal_wait_until operates on uint64; rocSHMEM spells this
#     rocshmem_uint64_wait_until (generated by the WRAP_WAIT macro).
#   - nvshmemx_signal_op has no direct device-bitcode equivalent in rocSHMEM;
#     use rocshmem_uint64_atomic_set / rocshmem_uint64_atomic_add instead.
NVSHMEM_TO_ROCSHMEM_DEVICE_MAPPINGS = collections.OrderedDict([
    # Block/wg-scoped RMA operations
    ...
    # Wait operations (typed wait_until variants)
    ...
    # PE identification
    ("nvshmem_my_pe",                 "rocshmem_my_pe"),
    ("nvshmem_n_pes",                 "rocshmem_n_pes"),
    # Collective synchronization
    ("nvshmem_barrier_all",           "rocshmem_barrier_all"),
    ("nvshmem_sync_all",              "rocshmem_sync_all"),
])
